<%@page import="dda.silabo.autentificarse.iu.LoginIU"%>
<%@page import="dda.silabo.menulateral.MenuLateralIU"%>
<%@page import="dda.silabo.observaciones.iu.ObservacionesIU"%>
<%@page import="dda.silabo.observaciones.comunes.Observacion"%>
<%@page import="dda.silabo.estructura.unidad.recursos.iu.RecursosIU"%>
<%@page import="java.util.List"%>
<%@page contentType="text/html" pageEncoding="UTF-8"%>

<%
    LoginIU logueo = (LoginIU) session.getAttribute("logueo");
    String tipo = "";
    if (logueo != null) {
        tipo = "html";
    } else {
        tipo = "login/loginControlador.jsp";
    }
    String accion = request.getParameter("accion");
    String menuLateral = "";
    RecursosIU recursos = (RecursosIU) session.getAttribute("Recursos");
    if (accion.equals("save")) {
        MenuLateralIU menu = (MenuLateralIU) session.getAttribute("MenuLateral");
        menuLateral = menu.toHTMLDocente(recursos.getSilabos().getIdUnidad(), logueo.getRolActivo());
    }
    String result = "{"
            + "\"contenidoTitulo\":\"" + recursos.getTitulo() + "\","
            + "\"contenidoDinamico\":\"" + recursos.toHTML() + "\","
            + "\"tipo\":\"" + tipo + "\","
            + "\"seleccionados\":\"" + recursos.seleccionadostoHTML() + "\","
            + "\"contenidoMenuLateral\":\"" + menuLateral + "\","
            + "\"lnkAyuda\":\"" + recursos.getAyuda() + "\""
            + "}";

    response.setContentType("text/plain");
    response.setCharacterEncoding("UTF-8");
    response.getWriter().write(result);
%>